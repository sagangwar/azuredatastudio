...
export class ManageAction extends Action {
	...
	override async run(actionContext: ManageActionContext): Promise<void> {
		if (actionContext.profile) {
			await this._connectionManagementService.connect(actionContext.profile, 'dashboard', actionContext.uri, { showDashboard: true, saveTheConnection: false, params: undefined, showConnectionDialogOnError: false, showFirewallRuleOnError: true });
